...
<div class="p-6">
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        <div class="bg-white rounded-xl shadow-lg p-6">
            ...
            @endif
            <a href="#" class="mt-3 inline-block text-purple-600">+ Start Workout (Coming Soon)</a>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            ...
            @endif
            <a href="#" class="mt-3 inline-block text-purple-600">+ Book a Trainer (Coming Soon)</a>
        </div>
    </div>
    
    <div class="mt-6">
        <div class="bg-white rounded-xl shadow-lg p-6">
            <h2 class="text-lg font-bold mb-4">Available Trainers</h2>
                ...
                @if(isset($availableTrainers) && $availableTrainers->count() > 0)
                    @foreach($availableTrainers as $trainer)
                        <div class="border rounded-lg p-4 text-center">
                            ...
                            <p class="text-sm text-gray-500">₹{{ $trainer->hourly_rate ?? 500 }}/hr</p>
                            <a href="#" class="mt-2 inline-block bg-purple-600 text-white px-4 py-1 rounded-lg text-sm">Book Now (Soon)</a>
                        </div>
                    @endforeach
                @else
                    <p class="text-gray-500 col-span-3">No trainers available</p>
                @endif
            </div>
        </div>
    </div>
